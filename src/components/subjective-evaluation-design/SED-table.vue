<template>
	<div class="SED_table">
		...
		<div class="table">
			<div class="title">
				<span>评估日期
					<img @click="_sortList('date')" src="../../../static/sort.png" style="height: 16px;vertical-align:middle;position: relative;bottom: 1px;right: 3px;">
				</span>
				<span>车辆编号
					<img @click="_sortList('modelNum')" src="../../../static/sort.png" style="height: 16px;vertical-align:middle;position: relative;bottom: 1px;right: 3px;">
				</span>
				<span>评估工况
					<img @click="_sortList('evaluationCon')" src="../../../static/sort.png" style="height: 16px;vertical-align:middle;position: relative;bottom: 1px;right: 3px;">
				</span>
				<span>参与人数
					<img @click="_sortList('personNum')" src="../../../static/sort.png" style="height: 16px;vertical-align:middle;position: relative;bottom: 1px;right: 3px;">
				</span>
			</div>
		...
		<div class="btn">
			...
			<div @click="modalShow = true" v-show="zzzShow">
		...
		<Modal style="z-index: 10000;" v-model="modalShow" title="提示" @on-ok="toClosePro" @on-cancel="modalShow = false">
			<p>实测数据和评估数据都已录入,确认关闭？</p>
		</Modal>
	</div>
</template>
		...
		methods: {
			_sortList(item) {
				...
				if (this.sortState === 0) {
					this.axios({
						method: 'get',
						url: `/patac_tse/thermalPropertyNoClose/${item}/0`,
						headers: {
					...
					this.axios({
						method: 'get',
						url: `/patac_tse/thermalPropertyNoClose/${item}/1`,
						headers: {
							'Content-type': 'application/json;charset=UTF-8'
						}
					}).then((res) => {
			...
			_getData() {
				this.axios({
					method: 'get',
					url: `/patac_tse/thermalPropertyNoClose/date/0`,
					headers: {
						'Content-type': 'application/json;charset=UTF-8'
					}
				}).then((res) => {
